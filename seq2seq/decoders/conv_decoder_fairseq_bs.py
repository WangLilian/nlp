...
class ConvDecoderFairseqBS(Decoder, GraphModule, Configurable):
  ...
  def step(self, time, inputs, state, batch_size=None, name=None, sample=False):
    ...
    logits = self.infer_conv_block(enc_output, cur_inputs_pos, batch_size)
  ...
  def infer_conv_block(self, enc_output, input_embed, batch_size):
    ...
    ###logits = tf.add(logits_message,logits_topic*topic_words_mask)   ###add mode
  ...
  def init_params_in_loop(self):
    with tf.variable_scope("decoder"):
      ...
      batch_size = self.config.beam_width###
      logits = self.infer_conv_block(enc_output, initial_inputs, batch_size)
