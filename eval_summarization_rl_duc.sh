...
export TEST_SOURCES=${DATA_PATH}/DUC2003/input.txt
  ...
  --input_pipeline "
    ...
    params:
      source_files:
        - $TEST_SOURCES" \
  > ${PRED_DIR}/summaryA.txt
rm ${BEAM_DIR}/summaryA.txt
cp ${PRED_DIR}/summaryA.txt ${BEAM_DIR}/summaryA.txt
rm ${PRED_DIR}/summaryA.txt

echo "Greedy result:" 
python seq2seq/metrics/pythonrouge/rouge_scorer.py -ref_dir $DATA_PATH/reference/2003 -sum_dir ${GREEDY_DIR}
echo "Beam result:"
python seq2seq/metrics/pythonrouge/rouge_scorer.py -ref_dir $DATA_PATH/reference/2003 -sum_dir ${BEAM_DIR}
